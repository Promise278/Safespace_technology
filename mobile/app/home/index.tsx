    ...
    <ScrollView className="flex-1 bg-[#f9f4f2] px-4 pt-16">
      ...
      <View className="flex-row items-center justify-between mb-6">
        <View className="flex-row items-center">
          <View className="w-10 h-10 rounded-full bg-[#f19469] items-center justify-center">
            <Ionicons name="shield" size={22} color="white" />
          </View>
          <View className="ml-3">
            <Text className="text-xl font-bold text-neutral-800">SafeSpace</Text>
            <Text className="text-sm text-neutral-500">Welcome, {username}</Text>
          </View>
        </View>

        {/* Icons Right */}
        <View className="flex-row items-center gap-5">
          <Feather name="home" size={18} color="#ed835f" />
          <TouchableOpacity onPress={() => router.push("/messages" as any)}>
             <Feather name="message-square" size={18} color="#444" />
          </TouchableOpacity>
      ...
      >
        <Text className="text-white font-semibold text-lg">+  Share Your Story</Text>
      </TouchableOpacity>
      ...
      {loading ? (
        ...
        <View className="pb-20">
          {stories.map((story) => (
             <View key={story.id} className="bg-white rounded-xl p-5 shadow-sm mb-4">
               {/* Header */}
               <View className="flex-row items-center mb-3">
                 ...
                 <View className="ml-3">
                   <Text className="text-base font-semibold text-neutral-800">{story.username}</Text>
                   <Text className="text-xs text-neutral-500">{timeAgo(story.createdAt)}</Text>
                 </View>
               </View>

               {/* Title */}
               <Text className="text-lg font-semibold text-neutral-900 mb-2">{story.title}</Text>
               ...
               <View className="flex-row items-center gap-8 border-t border-neutral-100 pt-3">
                 ...
                 >
                   ...
                   />
                   <Text className={`text-neutral-700 ${story.isSupported ? 'text-[#ed835f]' : ''}`}>
                      Likes {story.supportsCount ? `(${story.supportsCount})` : ''}
                   </Text>
                 </TouchableOpacity>
                 ...
                 >
                   <MaterialCommunityIcons name="message-outline" size={20} color="#444" />
                   <Text className="text-neutral-700">Respond</Text>
                 </TouchableOpacity>

                  {role === "supporter" && story.userId !== (AsyncStorage.getItem("safespace_user") as any)?.id && (
                    <TouchableOpacity 
                      className="flex-row items-center gap-2"
